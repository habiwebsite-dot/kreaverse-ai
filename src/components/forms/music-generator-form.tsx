      ...
      <Card>
        ...
        <div className="mt-6 grid gap-4">
          ...
          {[
            ...
            ['Weirdness constraint', weirdness, setWeirdness],
            ['Audio weight', audioWeight, setAudioWeight],
          ].map(([label, value, setter]) => (
            <label key={label} className="grid gap-2 text-sm">
              <div className="flex items-center justify-between"><span>{label}</span><span>{Number(value).toFixed(1)}</span></div>
              <input type="range" min={0} max={1} step={0.1} value={Number(value)} onChange={(e) => (setter as (v:number)=>void)(Number(e.target.value))} />
            </label>
          ))}
          <Button onClick={handleSubmit} disabled={loading}>{loading ? 'Mengirim task...' : 'Generate Musik'}</Button>
