...
async function createTask(params) {
  ...
  const response = await axios.post('https://api.aimastering.com/v1/master', rest, { headers: { 'Authorization': `Bearer ${key}`, 'Content-Type': 'application/json' } });
...
async function getTaskResult(taskId, apiKeyParam) {
  ...
  const response = await axios.get(`https://api.aimastering.com/v1/master/${taskId}`, { headers: { 'Authorization': `Bearer ${key}` } });
