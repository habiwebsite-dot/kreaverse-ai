...
router.post('/createTask', async (req, res) => {
  ...
  try {
    // Sertakan flag usePersonalKey jika ingin menggunakan API key pribadi
    // Di dalam model, jika usePersonalKey true, akan mencari dari database
    const taskInput = { ...input };
    if (usePersonalKey) {
      // Ambil personal key dari database dan sertakan sebagai apiKey
      const { getActiveApiKey } = require('../utils/api-key-manager');
      const personalKey = getActiveApiKey(req.user, provider, true);
      if (personalKey) taskInput.apiKey = personalKey;
    }
    const taskResult = await modelHandler.createTask(taskInput);
    const db = getDb();
    ...
    return success(res, {
      taskId: info.lastInsertRowid.toString(),
      state: taskResult.state || 0
    });
  } catch (err) {
...
router.get('/recordInfo', async (req, res) => {
  ...
  try {
    ...
    db.prepare('UPDATE tasks SET state = ?, result_json = ? WHERE id = ?')
      .run(result.state, JSON.stringify(result.resultUrls || result.result || null), task.id);
